...
class Parser
{
    ...
    const POSITION_STREAM = 'stream';
    const POSITION_OBJECT = 'object';
    ...
    public function __construct($stream, $listener, $lineEnding = "\n", $emitWhitespace = false, $bufferSize = 8192)
    {
        ...
        $this->emitAlterLine = method_exists($this->listener, 'readLineAlter');
    ...
    public function parse()
    {
        ...
        while (!feof($this->stream) && !$eof && !$this->stopParsing) {
            $pos = $this->getPosition();
            $line = stream_get_line($this->stream, $this->bufferSize, $this->lineEnding);
            if ($this->emitAlterLine) {
              $this->listener->readLineAlter($line);
            }
            
            $ended = (bool)($this->getPosition() - strlen($line) - $pos);
            // if we're still at the same place after stream_get_line, we're done
            $eof = $this->getPosition() == $pos;

            $byteLen = strlen($line);
            for ($i = 0; $i < $byteLen; $i++) {
                $this->currentByte = $pos + $i;
    ...
    private function endArray()
    {
        ...
        $this->latestPos[static::POSITION_ARRAY] = $this->currentByte;
    }
    ...
    private function endObject()
    {
        ...
        $this->latestPos[static::POSITION_OBJECT] = $this->currentByte;
    }
    ...
    public function getPosition($type = NULL) {
      if ($type !== null) {
        switch ($type) {
          case static::POSITION_OBJECT:
          case static::POSITION_ARRAY:
            return $this->latestPos[$type];
        }
      }
